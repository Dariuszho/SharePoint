...
.DESCRIPTION
    ...
    This version uses a local tracking file instead of a CAML query to
...
try {
    ...
    if (Test-Path -Path $ProcessedIDsFile) {
        $processedIDs = @(Get-Content -Path $ProcessedIDsFile | Where-Object { $_ -ne '' })
    }
    else {
        $processedIDs = @()
    }

    # Filter to only unprocessed items (client-side)
    $unprocessedItems = $items | Where-Object { $processedIDs -notcontains $_.Id.ToString() }

    if (-not $unprocessedItems -or @($unprocessedItems).Count -eq 0) {
...
foreach ($item in $unprocessedItems) {
    try {
        ...
        # GroupsField is a Lookup/User field — extract the LookupValue string (e.g. "SNIFIM\jira_security")
        ...
        # ── Mark Item as Processed (local tracking file) ─────────────────
        try {
            Add-Content -Path $ProcessedIDsFile -Value $item.Id.ToString()
        }
        catch {
            Write-Log -Message "Error updating processed IDs file for item ID $($item.Id): $_" -Level ERROR
        }
